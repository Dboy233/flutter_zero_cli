...
/// （调用方据此打印分级日志）。本函数不负责 import 与格式化。
Future<ToastHandlePatchOutcome> patchDefaultToastHandle(
  File file, {
  bool force = false,
}) async {
  ...
  if (fn.isEmpty) {
    return (result: ToastHandlePatchResult.anchorNotFound, replacedSource: null);
  }
  ...
  if (blockSource.contains(wiredMarker)) {
    return (result: ToastHandlePatchResult.alreadyWired, replacedSource: null);
  }
  final isTemplate = blockSource.contains(_templateMarker);
  if (!isTemplate && !force) {
    return (result: ToastHandlePatchResult.customSkipped, replacedSource: null);
  }
...
Future<void> formatDartFile(File file) async {
  final source = await file.readAsString();
  try {
