...
/// l10n 支持文件代码生成器。
///
/// 三个生成方法均为纯静态、可单测；生成源码统一由 dart_style 格式化。
class L10nCodeGenerator {
  /// 生成 `l10n_code.dart` 内容。
  static String generateL10nCode(List<L10nMember> members) {
  ...
  static String generateL10nCodeExt(String packageName) {
  ...
  static String generateL10nToastEffectHelper(
  ...
  static String _deserializeExpr(L10nParam p) =>
      p.paramType.deserializeExpr("l10nCode.parameters['${p.name}']");

  /// 文档示例中使用的参数示例值。
  ///
  /// 委托给 [L10nParam.paramType]，未知类型回退为 `value`。
  static String _exampleValue(L10nParam p) => p.paramType.exampleValue();
