...
- 移除了 `new` 与 `gen-l10n` 命令的 CLI/模板版本兼容门禁。这两个命令不再读取项目 `flutter_zero_config.yaml` 中的 `minCliVersion` 字段来拒绝或放行某个 CLI 版本；原先仅用于跳过该门禁的 `--skip-version-check` 调试参数也随之失效并移除。

### 变更

- `create` 命令现在始终选取可用的最新模板版本，而不再按最低 CLI 版本过滤候选模板。
- 优化并统一了模板源解析、缓存操作与版本更新提示处的多语言（zh/en/ja）文案，清理了过时注释。

## 1.2.1

### 修复

- `create` 命令不再删除与待创建项目同名的既有目录。此前命令会先报告名称冲突，随后递归删除该目录及其全部内容；现在仅报错退出，不会改动你的任何文件。若 `create` 的后续步骤失败，本次命令自己创建的半成品项目目录仍会被清理。

## 1.2.0

### 新增

- 终端 CLI 新增多语言支持：提供中文（zh）、英文（en）、日文（ja）三种界面语言。新增全局 `--locale`/`-L` 标志指定运行语言；未指定时自动跟随系统区域或环境变量。
- `new` 与 `gen-l10n` 命令在配置校验失败时的错误提示现已本地化（zh/en/ja），不再为硬编码的中英混排文本。

### 变更

- 子进程的标准输出与错误输出现在并发排空，消除了此前管道背压导致的卡顿——未加 `--log` 时执行子命令（尤其是 `new` 中的 `build_runner`）会明显变慢。
- 执行子命令时默认关闭其标准输入。子进程若等待交互式输入（如确认提示）将不再静默挂起，而是直接采用默认行为。
