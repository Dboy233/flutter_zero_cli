...
void main() {
  ...
  group('patchDefaultToastHandle', () {
    test('模板态（assert 兜底）→ patched，分支被替换', () async {
      ...
      final outcome = await patchDefaultToastHandle(handleFile);
    ...
    test('模板态替换不触碰同文件其他开发者代码（误触防护）', () async {
      ...
      await patchDefaultToastHandle(handleFile);
    ...
    test('自定义态 + force → patched', () async {
      ...
      final outcome = await patchDefaultToastHandle(handleFile, force: true);
